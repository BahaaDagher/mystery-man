...
import BranchesEvaluationInRelationToTime from './branch_evaluation_in_relation_to_time/BranchesEvaluationInRelationToTime'
import StepsEvaluationInRelationToTime from './steps-evaluate-in-relation-to-time/StepsEvaluationInRelationToTime'
import GeneralRate from '../one-branch-report/general-rate/GeneralRate'
import ReviewsQualification from '../one-branch-report/reviews-qualification/ReviewsQualification'

// This component mirrors MoreThanBranchReport but is used for the QR report flow.
const QrMoreThanBranchReport = ({moreThanBranchData, onStepsIdsChangeFromMoreThanBranch, allSteps, selectedQrBranches}) => {
  ...
  const isSingleBranch = Array.isArray(selectedQrBranches) && selectedQrBranches.length === 1
  return (
    <div className='flex flex-col gap-3'>
      {isSingleBranch ? (
        <div className='w-full flex flex-col lg:flex-row justify-between gap-3 pdf-section'>
          <div className='w-full lg:w-[35%] bg-white rounded-[12px] '> <GeneralRate apiData={moreThanBranchData?.generalRate}/></div>
          <div className='w-full lg:w-[65%] bg-white rounded-[12px] '> <ReviewsQualification apiData={moreThanBranchData?.reviewStats}/></div>
        </div>
      ) : (
        <div className='w-full flex flex-col lg:flex-row justify-between gap-3 pdf-section'>
      ...
      <div className='w-full bg-white rounded-[12px] pdf-section'> <BranchesEvaluationInRelationToTime apiData={moreThanBranchData?.branchImprovementTrend}/></div>
      <div className='w-full bg-white rounded-[12px] pdf-section'> <StepsEvaluationInRelationToTime apiData={moreThanBranchData?.stepMonthlyTrendPerBranch}/></div>
      <div className='w-full flex flex-col lg:flex-row justify-between gap-3 pdf-section'>
...
export default QrMoreThanBranchReport
