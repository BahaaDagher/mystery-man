...
import {
  ...
  setNewStep,
  setNewStepName,
} from "../../store/slices/questionierSlice";
...
const QuestionnaireSettings = ({ isAddNew }) => {
  ...
  const questionierDataSent = useSelector(
    (state) => state.questioneirData.questionierDataSent
  );
  const questionierDataDelete = useSelector(
    (state) => state.questioneirData.questionierDataDelete
  );
  const questionieres = useSelector(
    (state) => state.questioneirData.questionieres
  );
  const currentQuestioneir = useSelector(
    (state) => state.questioneirData.currentQuestioneir
  );
  const currentStep = useSelector((state) => state.questioneirData.currentStep);
  const isReadyToSend = useSelector(
    (state) => state.questioneirData.isReadyToSend
  );
  ...
  const getQuestionnaireLoading = useSelector(
    (state) => state.questioneirData.getQuestionnaireLoading
  );
  ...
  useEffect(() => {
    if (questionierDataSent && pressSave) {
      Swal.fire(questionierDataSent.message, "", "success").then((result) => {
  ...
  useEffect(() => {
    if (questionierDataDelete && pressSave) {
      Swal.fire(questionierDataDelete.message, "", "success").then((result) => {
        if (result.isConfirmed) {
          window.location.reload();
        }
      });
    ...
    <>
      ...
      {getQuestionnaireLoading ? (
        ...
        <Parent>
          <Settings>
            ...
            <InputAndButtons>
              ...
              <ButtonsContainer>
                ...
                <ActionButton onClick={() => handleSaveQuestioneir()}>
                  {t("text.Save")}
                </ActionButton>
                <ActionButton
                  onClick={() => handleDeleteQuestioneir()}
                  className="cancel"
                >
                  {t("text.Delete")}
                </ActionButton>
            ...
            <DndProvider backend={HTML5Backend}>
              <StepsContainer style={{ justifyContent: "start" }}>
                {questionieres[currentQuestioneir]?.steps?.map(
                  (answer, index) => (
                    <AddStepButton
                      className={activeStep == index ? "active" : ""}
                    >
          ...
          <QuestionView>
            {questionieres?.[currentQuestioneir]?.steps?.[currentStep]?.questions
              .length > 0 ? (
              <DndProvider backend={HTML5Backend}>
                <QuestionComponent
                  setIsApplyFocus={setIsApplyFocus}
                  questions={
                    questionieres?.[currentQuestioneir]?.steps?.[currentStep]
                      ?.questions
                  }
                ></QuestionComponent>
              </DndProvider>
